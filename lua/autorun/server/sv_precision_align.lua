...
-- Exceptions lists - all lower casenot 
-- General
...
function precision_align_rotate_func( ply, cmd, args )
	...
	if not tonumber(args[1]) or !tonumber(args[2]) or !tonumber(args[3]) then
		return false
	end
	...
	-- Get pivot point
	local v
	if not tonumber(args[4]) or !tonumber(args[5]) or !tonumber(args[6]) then
		v = startpos
	else
...
function precision_align_constraint_func( len, ply )
	...
	if CPPI then
		if Ent1:IsWorld() then
			if not util.IsValidPhysicsObject(Ent2, 0) or !Ent2:CPPICanTool( ply, PA ) then
				return false
			end
		elseif Ent2:IsWorld() then
			if not util.IsValidPhysicsObject(Ent1, 0) or !Ent1:CPPICanTool( ply, PA ) then
				return false
			end
		elseif not util.IsValidPhysicsObject(Ent1, 0) or !util.IsValidPhysicsObject(Ent2, 0) then
			return false
		elseif not Ent1:CPPICanTool( ply, PA ) or !Ent2:CPPICanTool( ply, PA ) then
			return false
		end
	else
