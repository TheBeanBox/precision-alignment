...
PA_funcs.line_global = function( line )
	...
	if not line_temp.startpoint or !line_temp.endpoint then
		return false
	end
...
PA_funcs.set_line = function( line, startpoint, endpoint, direction, length )
	...
	if startpoint then
		...
		if not endpoint and !direction then
...
PA_funcs.attach_line = function( line, ent )
	if not precision_align_lines[line].startpoint or !precision_align_lines[line].endpoint then
...
PA_funcs.plane_global = function( plane )
	...
	if not plane_temp.origin or !plane_temp.normal then
		return false
	end
...
PA_funcs.attach_plane = function( plane, ent )
	if not precision_align_planes[plane].origin or !precision_align_planes[plane].normal then
...
local function solve_point_2line_intersection( line1, line2 )
	...
	if not length1 or !length2 then return false end
...
PA_funcs.line_plane_projection = function( lineID, planeID )
	...
	if line.startpoint == line.endpoint then
		Warning("Line - plane projection results in a single pointnot  Check line is not perpendicular to plane")
		return false
	end
...
PA_funcs.rotate_line_plane_parallel = function( pivot, axis, lineID, planeID, activeent )
	...
	if axis then
		...
		elseif math.asin(axisdir:Dot(normal)) > math.acos(axisdir:Dot(linedir1)) then
			Warning("Line cannot be rotated parallel to plane along this axisnot ")
			return false
		end
	else
